...
class controleAcessoDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = controleAcessoUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "controleAcessoDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = controleAcessoUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = controleAcessoUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = controleAcessoUser.findByUsername(principal)
        ...
        def results = controleAcessoUser.executeQuery("select distinct p from controleAcessoUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
